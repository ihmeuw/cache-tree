...
var CacheTree = function () {
  function CacheTree(hierarchy) {
    ...
    // LRU-LINKED-LIST
    this._lru = new _linkedlist.LinkedList(this._maxSize);
  }
  ...
  _createClass(CacheTree, [{
    ...
    value: function _search(path, cache, filter) {
      ...
      if (path.length === 0) {
        ...
        return (0, _lodash.castArray)(cache.key);
        ...
        var trimmedCache = (0, _lodash.pick)(cache, filter[pathNode]);

        return (0, _lodash.flatMap)(trimmedCache, function (subCache) {
          return _this._search(pathRemaining, subCache, filter);
        });
      } else if ((0, _lodash.has)(filter, pathNode)) {
        if (!(0, _lodash.has)(cache, filter[pathNode])) {
          return [];
        }
      ...
      return (0, _lodash.flatMap)(cache, function (subCache) {
        return _this._search(pathRemaining, subCache, filter);
      });
    }
  }, {
    ...
    value: function _insert(path, subCache, data) {
      ...
      if (path.length && !(0, _lodash.has)(data, pathNode)) {
        throw new Error('missing property in data');
      }

      if (path.length === 0) {
        if (!(0, _lodash.isEmpty)(subCache)) {
        ...
        var node = new _linkedlist.ListNode(Object.freeze(data));
      ...
      if (!(0, _lodash.has)(subCache, data[pathNode])) {
    ...
    value: function _check(path, cache, filter) {
      ...
      if (path.length === 0) {
        ...
        return (0, _lodash.every)(filter[pathNode], function (param) {
          return (0, _lodash.has)(cache, param) && _this2._check(pathRemaining, cache[param], filter);
        });
      } else if ((0, _lodash.has)(filter, pathNode) && (0, _lodash.has)(cache, filter[pathNode])) {
    ...
    value: function _size(path, cache) {
      ...
      return (0, _lodash.reduce)(cache, function (acc, subCache) {
    ...
    value: function _diff(path, cache, filter) {
      ...
      if (path.length === 0) {
        ...
        var filterPieces = (0, _lodash.map)(filter[pathNode], function (param) {
          if ((0, _lodash.has)(cache, param)) {
            var subDiff = _this4._diff(pathRemaining, cache[param], (0, _lodash.omit)(filter, pathNode));

            if ((0, _lodash.isEmpty)(subDiff)) return subDiff;

            return (0, _lodash.assign)({}, _defineProperty({}, pathNode, param), subDiff);
          }

          return (0, _lodash.assign)({}, _defineProperty({}, pathNode, [param]), (0, _lodash.omit)(filter, pathNode));
        });

        return (0, _lodash.reduce)(filterPieces, function (acc, partial) {
          return (0, _lodash.mergeWith)(acc, partial, function (accValue, partialValue) {
            return (0, _lodash.compact)((0, _lodash.union)((0, _lodash.castArray)(accValue), (0, _lodash.castArray)(partialValue)));
          });
        }, {});
      } else if ((0, _lodash.has)(filter, pathNode) && (0, _lodash.has)(cache, filter[pathNode])) {
        var subDiff = this._diff(pathRemaining, cache[filter[pathNode]], (0, _lodash.omit)(filter, pathNode));

        if ((0, _lodash.isEmpty)(subDiff)) return subDiff;

        return (0, _lodash.assign)({}, _defineProperty({}, pathNode, filter[pathNode]), subDiff);
      }
    ...
    value: function _remove(path, cache, data) {
      ...
      } else if (this._remove(pathRemaining, cache[data[pathNode]], data) && (0, _lodash.isEmpty)(cache[data[pathNode]])) {
    ...
    value: function set(data) {
      ...
      if ((0, _lodash.isArray)(data)) {
        (0, _lodash.forEach)(data, function (datum) {
    ...
    value: function clone() {
      ...
      return (0, _lodash.assign)(newCache, this);
    }
    ...
    value: function extract(filter) {
      ...
      (0, _lodash.forEach)(dataToBeExtracted, function (datum) {
    ...
    value: function getDiff(paramFilter) {
      return this._diff(this._structure, this._cache, (0, _lodash.pick)(paramFilter, this._structure));
    }
